...
def exportStudyList(database):
	os.system('clear')
	printHeader()
	if (len(database.patientDB.keys()) == 0):
		raw_input("Sorry, there don't appear to be any patients in the database yet. Try adding one!")
	else:
		studies = [study for study in database.patientDB[database.patientDB.keys()[0]].characteristics.keys() if study != 'basics']
		for study, idx in zip(studies, range(1, len(studies) + 1)):
			print "[", idx, "]", database.patientDB[database.patientDB.keys()[0]].characteristics[study]['ID'], "Eligible Patients"
		print "[", idx + 1, "]", "All Patients"
		print "[", idx + 2, "]", "Exit"
		uin = userInput("\nWhat patient list would you like to export? ", 'int')
		if (uin in range(1, len(studies) + 1)):
			study = studies[uin - 1]
			patientList = [MRN for MRN in database.patientDB.keys() if (database.patientDB[MRN].characteristics[study]['eligible'] == True)]
			if (len(patientList) == 0):
				raw_input("Sorry, there don't appear to be any patients for this study yet. Try adding one!")
				exportStudyList(database)
			else:
				basicDict = [criteria for criteria in database.patientDB[patientList[0]].characteristics['basics'].keys() if criteria not in ['ID', 'eligible']]
				studyDict = [criteria for criteria in database.patientDB[patientList[0]].characteristics[study].keys() if criteria not in ['ID', 'eligible']]
				with open(database.patientDB[patientList[0]].characteristics[study]['ID'] + " Study List.txt", 'w+') as f:
					f.write('MRN\t' + '\t'.join(basicDict + studyDict) + '\n')
					for MRN in patientList:
						f.write(MRN + '\t')
						f.write('\t'.join([str(database.patientDB[MRN].characteristics['basics'][criteria]) for criteria in basicDict]) + '\t')
						f.write('\t'.join([str(database.patientDB[MRN].characteristics[study][criteria]) for criteria in studyDict]) + '\n')
				raw_input("Patient list exported successfully!")
				os.system('clear')
		elif (uin == len(studies) + 1):
			patientList = database.patientDB.keys()
			if (len(patientList) == 0):
				raw_input("Sorry, there don't appear to be any patients yet. Try adding one!")
			studyList = database.patientDB[patientList[0]].characteristics.keys()
			studyDict = {study:[] for study in studyList}
			for study in studyList:
				for criteria in [criteria for criteria in database.patientDB[patientList[0]].characteristics[study].keys() if criteria not in ['ID', 'eligible']]:
						studyDict[study].append(criteria)
			with open("All Patients.txt", 'w+') as f:
				f.write('MRN\t')
				for study in studyDict.keys():
					criteriaList = studyDict[study]
					for criteria in criteriaList:
						f.write(criteria + "(" + database.patientDB[patientList[0]].characteristics[study]['ID'] + ")" + '\t')
				f.write('\n')
				for MRN in patientList:
					f.write(MRN + '\t')
					patientValues = [database.patientDB[MRN].characteristics[study][criteria] for study in studyDict.keys() for criteria in studyDict[study]]
					f.write('\t'.join([str(value) for value in patientValues]))
				raw_input("Patient list exported successfully!")
				os.system('clear')
		elif (uin == len(studies) + 2):
			os.system('clear')
			return		
		else:
...
def importStudyList(database):
	with open("All Patients.txt", 'r') as file:
		header = file.readline()
		patientValues = file.readlines()
...
def main(): 
	...
	while (menuOption != 3):
		if (menuOption == 1):
			...
			# while (MRN != -1):
			if (searchResult):
				study = patientMenu(database, MRN)
				os.system('clear')
				while (study != 'exit'):
